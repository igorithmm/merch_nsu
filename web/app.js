...
const HELP_SECTIONS = [
  ...
  ['Доступ с другого компьютера', `
    ...
    <p><b>Адрес может смениться.</b> Сеть обычно выдаёт компьютеру адрес на время,
       и после перезагрузки роутера он может стать другим — старая закладка у коллеги
       перестанет открываться. Актуальный адрес всегда виден здесь же, в настройках;
       код придётся ввести заново, потому что браузер привязывает вход к адресу.</p>
    ...
    <p><b>Что открывается, а что нет.</b> По сети доступен только учёт мерча: страница
       приложения и его данные. Добраться через приложение до других файлов на
       компьютере нельзя. Завершение работы, настройка доступа и смена кодов работают
       лишь за самим компьютером — по сети их не выполнить даже по коду продавца.
       Бэкап скачивает только продавец: в файле базы лежат и коды доступа.</p>`],
  ...
  ['Выгрузки в CSV', `
    <p>Три кнопки на вкладке «Настройки» и такие же на «Журнале» и «Отчётах». Файлы
       ...
       региональных настройках Windows. Если на компьютере стоит английский
       формат (запятая вместо точки с запятой), двойной клик всё перепутает —
       тогда открывайте файл через «Данные» → «Из текста/CSV» и укажите
       разделителем точку с запятой вручную.</p>
    <p><b>Остатки</b> — срез склада для сверки с полкой: категория, тип, цвет, принт,
       ...
       пропускаются. Снятое с продажи выгружается наравне со всем остальным и никак не
       помечается: на полке оно лежит, а значит и в пересчёте участвует. В столбце
       «Пересорт» стоит «есть», если у размера заполнено наименование 1С
       в «Отметках размеров».</p>
    <p><b>Журнал</b> учитывает выбранные фильтры — период, категорию, тип операции,
       продавца и поиск. <b>Желания</b> выгружаются целиком.</p>`],
];
