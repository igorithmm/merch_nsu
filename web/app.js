...
function renderFilters() {
  ...
  // Размеры — отдельной строкой, выбирать можно сразу несколько. В сувенирной
  // продукции размеров нет вовсе, и выбор любого из них прятал весь список:
  // «Ничего не нашлось» без единой подсказки, почему. В этой категории строку
  // не показываем и снимаем уже выбранное.
  const sizes = f.category === 'souvenir' ? [] : (facets.sizes || []);
  if (!sizes.length && f.sizes.length) f.sizes = [];
...
async function loadJournal() {
  ...
  // Записи на текущей странице могли кончиться: их удалили, откатили или
  // сузили фильтром. Пустой экран с надписью «Записей нет» при непустом журнале
  // читается как поломка, поэтому отходим на последнюю непустую страницу.
  if (!data.items.length && data.total && j.offset >= data.total) {
    j.offset = Math.max(0, (Math.ceil(data.total / j.limit) - 1) * j.limit);
    return loadJournal();
  }
...
function renderReports(rep) {
  ...
  $('#reportBody').innerHTML = `
    ...
    <div class="panel">
      ...
      <p class="panel__hint">Период ${fmtDate(s.date_from)} — ${fmtDate(s.date_to)}. «Хватит на» — при текущем темпе продаж.${
        s.deleted_ops ? ` В итогах сверху учтены ещё ${nf.format(s.deleted_qty)} ${plural(s.deleted_qty, 'штука', 'штуки', 'штук')}
        по товарам, которые с тех пор удалили: в этой таблице их нет — карточки уже не существует.` : ''}</p>
      <div class="table-wrap"><table class="table"><thead><tr>
...
const HELP_SECTIONS = [
  ...
  ['Карточка товара', `
    ...
    <p>Категорию можно поменять и потом. Перевести одежду в сувенирную продукцию
       приложение разрешит только при нулевых остатках по размерам: у сувенира одна
       строка остатка, и всё, что лежало по размерам, иначе перестало бы показываться.
       Если остаток есть, приложение назовёт размеры и попросит сначала продать
       или списать их.</p>
  ...
  ['Товары: архив и удаление', `
    ...
    <p><b>Удаление</b> работает всегда, даже если по товару уже были продажи. Записи
       ...
       добавляется отметка об удалении с числом сохранённых операций. Выручка за прошлые
       периоды тоже остаётся на месте — из таблиц по товарам позиция пропадёт, из итогов
       нет. Если сомневаетесь — лучше архив: он ничего не теряет.</p>`],
  ...
  ['Отчёты', `
    ...
    <p>Продажи удалённых товаров остаются в общих цифрах периода: выручка за прошлый
       месяц не меняется задним числом. В таблицах по товарам такой записи не будет —
       карточки уже нет, — а сколько штук на них пришлось, написано под заголовком
       «Что продаётся».</p>`],
